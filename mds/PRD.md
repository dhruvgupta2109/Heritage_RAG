...
**Status:** v1.2 — Phase 1 implemented
**Owner:** You
...
- Dropdown/pill in the input bar. v1 candidates:
  ...
  - **OpenAI** (e.g. GPT-4.1 / GPT-4o class model)
  - **Gemini** (Google, e.g. Gemini 2.x)
...
| LLM providers | Groq for Phase 1; OpenAI/Gemini and optional Anthropic/Ollama later | Starts with the available key while preserving the adapter design |
...
2. **Multi-model + Speed modes** — provider adapter layer, speed-mode retrieval logic.
3. **Password-protected upload** — add-to-index flow from the UI.
4. **Chat history** — sidebar, persistence, rename/delete.
...
- Groq is confirmed for Phase 1. Which additional provider should be added first in Phase 2?
...
- One shared upload password is assumed sufficient — confirm that's fine vs. wanting per-user accounts later.

---

*Next step: once you confirm the open questions (or are fine with the defaults above), this PRD can be turned into a working scaffold — backend + frontend + ingestion script — ready to run on localhost.*
